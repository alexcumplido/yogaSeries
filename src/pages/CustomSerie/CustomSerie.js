...
export function CustomSerie() {
  ...
  function storageCustomSerie() {
    window.localStorage.setItem(nameToStorage.nameStorage, JSON.stringify(customSerie));
    setCustomSerie([]);
  }

  function retrieveCustomSerie() {
    if(window.localStorage.getItem(nameToRetrieve.nameRetrieve)) {
     setCustomSerie(JSON.parse(window.localStorage.getItem(nameToRetrieve.nameRetrieve))); 
     alert("Serie loaded");
    }
  }

  let listItems = response && response.items.map((element, idx) => {
    return (
      ...
      <div className="wrapper__search">
        ...
        <div className="retrieve">
          <input 
            ...
            minLength='4'
          />
          <button onClick={() => retrieveCustomSerie()}>
              Load
          </button>
          
          </div>
       <Link to={customSerie.length>0 ? `/serie` : '/customSerie'} state={customSerie}> Start serie </Link>
     
      </div>
      <ul className="list__links__custom">
        {listItems}
      </ul>
    </div>
  );
